    ...
    switch (type) {
        case 3:
            ...
            if (strcmp(pp->sni, "No SNI") != 0){
                ...
                "Packets:%d\n"
                "Duration: %.3f\n",
                log_count, tmp, pp->time_stamp.tv_usec, source_ip, ntohs(pp->src_port),ntohs(pp->dst_port),
...
bool cmp_ip6(struct in6_addr client_src, struct in6_addr client_dst, struct in6_addr server_src, struct in6_addr server_dst) {
    if (memcmp(&client_src, &server_src, sizeof(struct in6_addr)) && memcmp(&client_dst, &server_dst, sizeof(struct in6_addr))){
        return true;
    }
    return false;
}

tls_connection *get_conn_6(const struct ip6_hdr *ip6_header, const struct tcphdr *tcp_header){
    tls_connection *pp = connections;
    while (pp != NULL){
        if (pp->addr_size == INET6_ADDRSTRLEN){
            if (cmp_ip6(pp->ip6_src, pp->ip6_dst, ip6_header->ip6_src, ip6_header->ip6_dst)){
                preprocess_packet(pp, true, tcp_header->fin);
                return pp;
            }
            if (cmp_ip6(pp->ip6_src, pp->ip6_dst, ip6_header->ip6_dst, ip6_header->ip6_src)) {
...
void process_tls(tls_connection *pp, u_char *payload, size_t size_of_data) {
    ...
    for (u_char *j = payload; j < (payload + size_of_data); j += tls_header.len + 5){
...
void packet_handler(u_char *userData, const struct pcap_pkthdr *pkt_hdr, const u_char *packet) {
    ...
    switch (type){
    case ETHERTYPE_IP:
        ip4_header = (struct iphdr *)(packet + sizeof(struct ether_header));
        tcp_header = (struct tcphdr *)(packet + sizeof(struct ether_header) +
                                       sizeof(struct iphdr));
        conn = get_conn(ip4_header, tcp_header);
        break;
    case ETHERTYPE_IPV6:
        ...
        break;
    default:
        // Nither IPv4 nor IPv6 
        return;
    }

    size_t size = 0;
    if (conn == NULL){
        // printf("Connetion is not found\n");

        conn = (tls_connection*)malloc(sizeof(tls_connection));
        if (type == ETHERTYPE_IP){
            size = sizeof(struct ethhdr) + ip4_header->ihl * 4 + tcp_header->doff * 4;
            // printf("Size is %ld\n", size);
            ...
            size = sizeof(struct ethhdr) + ip6_header->ip6_plen * 4 + tcp_header->doff * 4;
            conn->ip6_dst = ip6_header->ip6_src;
            conn->ip6_src = ip6_header->ip6_dst;
            conn->addr_size = INET6_ADDRSTRLEN;
        }
        else{
            printf("Nither IPv4 nor IPv6 packet, exit...\n");
            return;
        }
